...
def flatten_patches(patches):
    ...
    # [bs, num_patches, c, ps, ps] -> [bs, num_patches, -1]
...
def softmax(x):
    ...
    denum = torch.exp(x - max_x).sum(dim=-1, keepdim=True)
    return num / denum
...
def scaled_dot_product_attention(q, k, v, mask=None):
    ...
    # q: [bs, n_h, len, d_head]
    attn_weights = q @ k.transpose(-1, -2) / math.sqrt(q.shape[-1])
    if mask is not None:
...
def merge_and_output_project(context_heads, wo, bo):
    ...
    return linear_projection(merge_heads(context_heads), wo, bo)
